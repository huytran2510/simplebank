...
func TestTransferAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			body: gin.H{
				"from_account_id": int64(account1.ID),
				"to_account_id":   int64(account2.ID),
				"amount":          amount,
				"currency":        util.USD,
			},
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
...
func TestTransferAPI2(t *testing.T) {
	amount := int64(10)
	...
	testCases := []struct {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				// Tạo một account không hợp lệ cho account2, nhưng vẫn cần trả về để hàm không panic
				invalidAccount := &db.Account{}

				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account2.ID)).Times(1).Return(invalidAccount, errors.New("not authorized"))
				store.EXPECT().TransferTx(gomock.Any(), gomock.Any()).Times(0)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
				require.Equal(t, http.StatusUnauthorized, recorder.Code)
		...
		// Add other test cases for edge cases here...
	}
